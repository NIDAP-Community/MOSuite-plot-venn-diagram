...
parser$add_argument(
  ...
  help = "Type of plot: 'Venn diagram' or 'Intersection plot'"
)
parser$add_argument(
  "--intersection_ids",
  type = "character",
  default = "",
  help = "Comma-separated intersection IDs to select"
)
parser$add_argument(
  "--venn_force_unique",
  type = "logical",
  default = TRUE,
  help = "Force unique elements in Venn diagram"
)
parser$add_argument(
  "--venn_numbers_format",
  type = "character",
  default = "raw",
  help = "Format for Venn numbers: 'raw', 'percent', 'raw-percent', or 'percent-raw'"
)
parser$add_argument(
  "--venn_significant_digits",
  type = "integer",
  default = 2,
  help = "Number of significant digits for Venn numbers"
)
parser$add_argument(
  "--venn_fill_colors",
  type = "character",
  default = "darkgoldenrod2,darkolivegreen2,mediumpurple3,darkorange2,lightgreen",
  help = "Comma-separated fill colors for Venn diagram"
)
parser$add_argument(
  "--venn_fill_transparency",
  type = "double",
  default = 0.2,
  help = "Transparency level for Venn fill colors"
)
parser$add_argument(
  "--venn_border_colors",
  type = "character",
  default = "fill colors",
  help = "Border colors for Venn categories"
)
parser$add_argument(
  "--venn_font_size_for_category_names",
  type = "double",
  default = 3,
  help = "Font size for category names in Venn diagram"
)
parser$add_argument(
  "--venn_category_names_distance",
  type = "character",
  default = "",
  help = "Distance of category names from Venn circles"
)
parser$add_argument(
  "--venn_category_names_position",
  type = "character",
  default = "",
  help = "Position of category names in Venn diagram"
)
parser$add_argument(
  "--venn_font_size_for_counts",
  type = "double",
  default = 6,
  help = "Font size for counts in Venn diagram"
)
parser$add_argument(
  "--venn_outer_margin",
  type = "double",
  default = 0,
  help = "Outer margin for Venn diagram"
)
parser$add_argument(
  "--intersections_order",
  type = "character",
  default = "degree",
  help = "Order of intersections: 'degree', 'freq', or other"
)
parser$add_argument(
  "--display_empty_intersections",
  type = "logical",
  default = FALSE,
  help = "Display empty intersections in plot"
)
parser$add_argument(
  "--intersection_bar_color",
  type = "character",
  default = "steelblue4",
  help = "Color for intersection bars"
)
parser$add_argument(
  "--intersection_point_size",
  type = "double",
  default = 2.2,
  help = "Size of points in intersection plot"
)
parser$add_argument(
  "--intersection_line_width",
  type = "double",
  default = 0.7,
  help = "Width of lines in intersection plot"
)
parser$add_argument(
  "--table_font_size",
  type = "double",
  default = 0.7,
  help = "Font size for table in plot"
)
parser$add_argument(
  "--table_content",
  type = "character",
  default = "all intersections",
  help = "Content of table: 'all intersections' or 'selected intersections'"
)
parser$add_argument(
  "--image_width",
  type = "integer",
...
venn_result <- plot_venn_diagram(
  ...
  venn_category_names_distance = parse_numeric_vector(
    args$venn_category_names_distance
  ),
  venn_category_names_position = parse_numeric_vector(
    args$venn_category_names_position
  ),
